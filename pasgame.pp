...
begin
    ...
    begin
        if PtrToItem^^.shape = TempShapePtr then
        begin
            tmp := PtrToItem^;
...
begin
    ...
    while tmp <> nil do
    begin
        if TempShapePtr = tmp^.shape then
        begin
            tmp := tmp^.next;
            continue;
        end;
        if CheckShapesCollision(TempShapePtr, tmp^.shape) then
        begin
            CheckCollision := true;
            exit;
        end;
...
procedure DisposeGameShapes;
var
    tmp, ListItem: ShapeItemPtr;
begin
    ListItem := ListOfShapes;
