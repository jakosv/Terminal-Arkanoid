...
const
    ...
    BonusesCount = 5;
    BonusProbability = 3; { 1 / BonusProbability }
...
procedure DisposeListOfBonuses;
var
    ListItem, tmp: BonusItemPtr;
begin
    ListItem := ListOfBonuses;
    while ListItem <> nil do
    begin
        tmp := ListItem;
        ListItem := ListItem^.next;
        RemoveShape(tmp^.BonusShape);
        dispose(tmp);
    end;
end;
...
var
    x, y: integer;
begin
    x := ball.BallShape^.x + ball.dx;
    y := ball.BallShape^.y + ball.dy;
    if (x <= 1) or (x >= GetFieldWidth) then
        ball.dx := -ball.dx;
    if (y <= 1) or (y >= GetFieldHeight) then
        ball.dy := -ball.dy;
end;
...
procedure DisposeListOfBalls;
var
    ListItem, tmp: BallItemPtr;
begin
    ListItem := ListOfBalls;
    while ListItem <> nil do
    begin
        tmp := ListItem;
        ListItem := ListItem^.next;
...
procedure HandleBonusCollisions(var TempBonus: ShapePtr; 
                                BonusType: BonusTypes);
var
    CollisionShape: ShapePtr;
begin
...
var
    tmp, bonus: BonusItemPtr;
begin
    bonus := ListOfBonuses;
    while bonus <> nil do
    begin
        MoveShape(bonus^.BonusShape, 0, 1);
        tmp := bonus;
        bonus := bonus^.next;
        HandleBonusCollisions(tmp^.BonusShape, tmp^.BonusType);
    end;
end;

function PlatformCollision(TempShape: ShapePtr): boolean;
begin
    PlatformCollision := CheckWallCollision(TempShape)
end;
...
procedure DisposeGame;
begin
    DisposeListOfBalls;
    DisposeListOfBonuses;
    DisposeGameShapes;
end;
